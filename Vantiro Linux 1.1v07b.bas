...
'THIS BUILD IS MADE FOR LINUX, WINDOWS WILL NOT BE GUARANTEED 100% TO WORK.
Const IHaveGoodPC = 1

Dim Shared Missing
Missing = _LoadImage("assets/missing.png", 32)
...
Dim Shared LastItemID As _Unsigned Integer
LastItemID = 1
Type Menu
    x1d As Double
    x2d As Double
...
Dim Shared InteractParticle
InteractParticle = LoadImage("assets/Vantiro-1.1v07b/items/ekeyicon.png", 33)
...
_Font BegsFontSizes(30)
For i = 1 To 2
    PlayerMember(i).autoBE = -1: PlayerMember(i).speedDiv = 1
Next
Dim Shared Noclip, HideUI, Debug
...
Do
    ...
    _Limit 65
    If _KeyDown(91) And delay = 0 Then ShowLight = 1: delay = 20
    DynamicLight(1).x = (CameraX * Map.TileSize) + Mouse.x
    DynamicLight(1).y = (CameraY * Map.TileSize) + Mouse.y
    ...
    ProceedCheck = Timer(.001)
    If NoAI = 0 Then GoSub OldZombieAI
    ...
    GoSub TriggerPlayer
    ProceedCheck = Timer(.001)
    If Config.Map_Lighting = 1 Then Lighting
    ...
    If _KeyDown(104) Then DynamicLight(1).detail = "High"
    If _KeyDown(108) Then DynamicLight(1).detail = "Low"
    If _KeyDown(109) Then DynamicLight(1).detail = "Medium"
    Print "LightStrength: "; DynamicLight(1).strength
    Print "LightDetail: "; DynamicLight(1).detail
    If _KeyDown(103) And delay = 0 Then
        For o = 1 To 9999999
            SpawnBloodParticle Player.x - 160, Player.y, Int(Rnd * 360) - 180, 20, "red"
        Next
    End If
    _Display
...
If Int(Rnd * 6) + 1 = 3 And DeathTimer < 400 Then
    SpawnBloodParticle Player.x - 20 + Int(Rnd * 21), Player.y - 20 + Int(Rnd * 21), -180 + Int(Rnd * 361), 20, "red"
    Part(LastPart).xm = Int(Rnd * 500) - 250
    Part(LastPart).ym = Int(Rnd * 500) - 250
    Part(LastPart).zm = Int(Part(LastPart).zm / 4)
End If
...
For i = 1 To ZombieMax
    If Zombie(i).active = 1 Then
        ...
        If Zombie(i).tick = 0 Then
            ...
            If Zombie(i).DistanceFromPlayer < (Zombie(i).size * 1.9) Then
...
Sub LoadWeapons
    Cls
    LastWeaponID = 1
    Open ("assets/Vantiro-1.1v07b/guns/active.gun") For Input As #9
    Line Input #9, trash$
    Do
        Line Input #9, Gun$
        Print Gun$
    ...
    For i = 1 To LastWeaponID
        Cls
        Print "Loading weapon: "; i
        Print ("assets/Vantiro-1.1v07b/guns/sprites/" + Weapons(i).BulletSpritePath + ".png")
        Print ("assets/Vantiro-1.1v07b/guns/sprites/" + Weapons(i).InternalName + ".png")
        Print ("assets/Vantiro-1.1v07b/guns/sprites/" + Weapons(i).AmmoSpritePath + ".png")
        Weapons(i).BulletSprite = LoadImage(("assets/Vantiro-1.1v07b/guns/sprites/" + Weapons(i).BulletSpritePath + ".png"), 33)
        Weapons(i).GunSprite = LoadImage(("assets/Vantiro-1.1v07b/guns/sprites/" + Weapons(i).InternalName + ".png"), 33)
        Weapons(i).AmmoSprite = LoadImage(("assets/Vantiro-1.1v07b/guns/sprites/" + Weapons(i).AmmoSpritePath + ".png"), 33)
    Next
End Sub

Sub LoadWeapon (ID As Long, Gun As String)
    Cls
    If _FileExists("assets/Vantiro-1.1v07b/guns/" + _Trim$(Gun) + ".gun") Then
        Weapons(ID).Exists = -1
        Open ("assets/Vantiro-1.1v07b/guns/" + _Trim$(Gun) + ".gun") For Input As #10
    ...
    Else
        ...
        Print ("File (" + _Trim$(Gun) + ".gun) was not found in 'assets/Vantiro-1.1v07b/guns'!")
    End If
    _Display
...
Sub SpawnItem (IType As String, IName As String, Extra1 As Double, Extra2 As Double)
    ...
    Else
        Print ("Type '" + _Trim$(IType) + "' isn't a valid type.")
        _Display
        _Delay 1
    End If
...
Sub VantiroConsole
    ...
    If UCase$(spawntype$) = "Z" Then
        ...
        Print "7- Fire"
        Input "Choose: ", Number
        If Number > 0 And Number < 8 Then SpawnZombie Number
    End If

    _Display
End Sub

Sub SpawnZombie (SelectedHud2ID As Integer)
    For z = 1 To ZombieMax
        If Zombie(z).active = 0 Then
            Zombie(z).active = 1
            Zombie(z).x = (CameraX * Map.TileSize) + Mouse.x
    ...
    For i = 0 To 32
        If Item(i).exist = -1 And Item(i).held = 0 Then
            ...
            If Dist < Player.size * 7 Then
                ...
                If Item(i).PickupV = -1 Then _PutImage (ETSX(Item(i).x) - 16, ETSY(Item(i).y) - 16)-(ETSX(Item(i).x) + 16, ETSY(Item(i).y) + 16), InteractParticle
            End If
        End If
    Next
...
Sub HandCollision (Arm As PlayerMembers)
    ...
    If Distance(Arm.x, Arm.y, Player.x, Player.y) > 500 Then Print "ARM OUT OF BOUNDSSSS": Arm.x = Player.x: Arm.y = Player.y
End Sub
    ...
    If Weapons(ID).Exists Then
        If It.Extra1 >= Weapons(ID).ShotsPerFire Then
            ...
            For b = 1 To Weapons(ID).ShotsPerFire
                ...
                Print spray
                BulletSpawn It.x, It.y, spray, 3, 2
...
Sub ParticleLogic (Part As Particle)
    Part.distancefromplayer = Distance(Player.x, Player.y, Part.x, Part.y)
    If Part.distancefromplayer > 900 Then Part.visible = Part.visible - 1
    ...
    If Part.z < 0 Then Part.zm = Part.zm * -.7: Part.z = 0: Part.xm = (Part.xm / 2): Part.ym = (Part.ym / 2)
    ...
    If Part.CanGrab And Part.distancefromplayer < BloodPickUpRadius Then
        ...
        If DistXGrab < _Width And DistYGrab < _Height Then
            ...
            Part.xm = Part.xm / 1.03
            Part.ym = Part.ym / 1.03
            Part.xm = Part.xm + -Sin(Part.rotation * PIDIV180) * 9 / (Part.distancefromplayer / 150)
            Part.ym = Part.ym + Cos(Part.rotation * PIDIV180) * 9 / (Part.distancefromplayer / 150)
            Part.z = 3 / (Part.distancefromplayer / 70)
            If Part.z > 15 Then Part.z = 15
            If Part.distancefromplayer < (Player.size / 2) Then PlayerPickUpParticle Part
...
Sub PlayerPickUpParticle (Part As Particle)
    ...
    Dim oldins As _Unsigned Integer
    Dim Commands As String
    Dim Value As String

    Do
        s_start = InStr(s_end, Part.WhenGrabbed, "{") + 1
        s_end = InStr(s_start, Part.WhenGrabbed, "}")
        s_com = InStr(s_start, Part.WhenGrabbed, ":")

        Commands = Mid$(Part.WhenGrabbed, s_start, s_com - s_start)
        Value = Mid$(Part.WhenGrabbed, s_com + 1, s_end - (s_com - 1))
...
Sub SpawnParticle (X As Double, Y As Double, Z As Double, XM As Double, YM As Double, ZM As Double, Rotation As Double, RotationSpeed As Double, PType As String)
    Dim LastPartBack As _Unsigned Integer
    LastPartBack = LastPart

    LastPart = LastPart + 1: If LastPart > ParticlesMax Then LastPart = 0
    Part(LastPart).froozen = 0
    Part(LastPart).visible = 0
...
Sub ParticleLogicHandler
    For i = 1 To ParticlesMax
        If Part(i).visible = 1 Then
            If Part(i).distancefromplayer < BloodPickUpRadius Then ParticleLogic Part(i)
            RenderParticles Part(i)
        End If
    Next
End Sub

Sub RenderParticles (Part As Particle)
    RotoZoomSized Part.x, Part.y, Part.SpriteHandle, Part.size, Part.rotation
    '(X AS LONG, Y AS LONG, Image AS LONG, Sc1 AS SINGLE, Rotation AS SINGLE)
End Sub
